...
// 5. Fuldt funktionsdygtig – kompilerer uden fejl efter sync. Test: UserRepositoryImpl → insertUser → getUserById (virker med FirmaUser).
...
import androidx.room.RewriteQueriesToDropUnusedColumns  // RETTET: Import for annotation (løser cursor-mismatch-varning)
...
interface UserDao {
    ...
    @RewriteQueriesToDropUnusedColumns  // RETTET: Tilføjet for at løse cursor-mismatch (Room rewrites query for unused columns som createdAt vs created_at)
    suspend fun getUserById(id: String): FirmaUser?

    @Query("SELECT * FROM users")
    @RewriteQueriesToDropUnusedColumns  // RETTET: Tilføjet for at løse cursor-mismatch
